...
test("completed attempts, perfect scores, and best score accumulate", () => {
  ...
  assert.deepEqual(first, { attempts: 1, perfect: 0, bestScore: 18, lastScore: 18, total: 20 })

  const second = nextNailExamGroupHistoryEntry(first, 20, 20)
  assert.deepEqual(second, { attempts: 2, perfect: 1, bestScore: 20, lastScore: 20, total: 20 })

  const third = nextNailExamGroupHistoryEntry(second, 17, 20)
  assert.deepEqual(third, { attempts: 3, perfect: 1, bestScore: 20, lastScore: 17, total: 20 })
})
...
test("storage parsing ignores corrupt payloads and invalid entries", () => {
  ...
  const mixed = memoryStorage({
    [NAIL_EXAM_GROUP_HISTORY_STORAGE_KEY]: JSON.stringify({
      "official:1-20": { attempts: 2, perfect: 1, bestScore: 20, lastScore: 20, total: 20 },
...
test("history writes and bank-scoped clearing preserve other banks", () => {
  ...
  const history = {
    "official:1-20": { attempts: 2, perfect: 1, bestScore: 20, lastScore: 20, total: 20 },
    "bank-a:1-20": { attempts: 3, perfect: 0, bestScore: 18, lastScore: 18, total: 20 },
  }
