...
export function ExamQuizChineseToggle({ placement = "fixed" }: { placement?: "fixed" | "inline" }) {
  const { showChinese, toggleChinese } = useExamQuizPreferences()
  ...
  return (
    ...
    >
      <span
        className="flex h-8 w-full items-center justify-center gap-1.5 rounded-full border border-border/80 bg-background/90 px-2 shadow-md backdrop-blur-sm transition-colors group-hover:bg-accent"
      >
