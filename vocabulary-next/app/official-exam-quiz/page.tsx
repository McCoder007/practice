...
import {
  ...
  sourceRangeCards,
} from "@/data/official-exam-quiz/catalog"
import { loadPracticeQuestions } from "@/data/official-exam-quiz/loadChapter"
import type {
  ExamAnswerRecord,
  ExamQuestion,
  ExamResultsSummary,
...
type OfficialExamQuizScreen = { step: "mode" } | { step: "quiz" } | { step: "results" }
...
function quizModeLabel(mode: QuizMode): { en: string; zh: string } {
  ...
  const source = PRACTICE_SOURCES.find((entry) => entry.id === mode.sourceId)
  if (!source) {
    return { en: "Unknown quiz", zh: "未知测验" }
  }
...
export default function OfficialExamQuizPage() {
  const { showChinese } = useOfficialExamQuizPreferences()
  const [screen, setScreen] = useState<OfficialExamQuizScreen>({ step: "mode" })
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      const nextScreen: OfficialExamQuizScreen =
        (event.state?.screen as OfficialExamQuizScreen | undefined) ?? { step: "mode" }
  ...
  useEffect(() => {
    ...
    window.history.pushState({ screen }, "", `#official-exam-quiz-${screen.step}`)
  }, [screen])
  ...
  const startQuiz = useCallback(async (mode: QuizMode) => {
    ...
    try {
      ...
      const selectedQuestions = isRandomQuizMode(mode)
        ...
        : sliceOfficialSourceRange(questions, mode.offset)
        ...
        <main className="flex h-dvh w-screen flex-col overflow-hidden bg-black">
          <header className="relative z-20 grid h-16 shrink-0 grid-cols-[3.25rem_minmax(0,1fr)_auto] grid-rows-2 items-center gap-x-2 border-b border-white/10 bg-black/70 px-3 py-2 text-center backdrop-blur-md">
            ...
            <div className="col-start-3 row-span-2 row-start-1 flex justify-end">
              <OfficialExamQuizChineseToggle placement="inline" />
            </div>
          </header>
