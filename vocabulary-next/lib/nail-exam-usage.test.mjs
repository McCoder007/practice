...
test("app activity keeps the last three distinct returns and throttles repeats", () => {
  const storage = memoryStorage()
  ...
  assert.deepEqual(record(4_000, 100), [4_000, 3_000, 2_000])
  assert.equal(storage.getItem(NAIL_EXAM_ACTIVITY_STORAGE_KEY), "[4000,3000,2000]")
})
...
test("activity throttle collapses overlapping calls into a single in-flight write", async () => {
  ...
  let resolveWrite
  let writeCount = 0
  const reportActivity = createActivityThrottle((learnerId, recentActiveAt) => {
    writeCount += 1
...
test("learner activity parsing requires newest activity first", () => {
  const activity = { learnerId: "learner-1", lastActiveAt: 3_000, recentActiveAt: [3_000, 2_000, 1_000] }
